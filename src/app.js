$(document).ready(function() {
  ...
  W_API.api_url = 'http://nodejs-opendimension.rhcloud.com/';
  //W_API.api_url = 'http://localhost:8080';

  // init leaflet
  W_LEAFLET.base_url = 'http://warper.wmflabs.org';
  W_LEAFLET.init();
  W_LEAFLET.setView(48.1619343018, 13.9516973282, 4); // Europe
  W_API.getAllMaps(map.getBounds(), showSearch);


  // make a geosearch every time when map stops moving
  W_LEAFLET.map.on('moveend', function() {
    if (W_LEAFLET.geo_search == 1) {
      W_API.geoSearch(map.getBounds(), updateMapList);
    }
  });
  ...
  var clustering = L.control({
    position: 'topleft'
  });
  clustering.onAdd = function(map) {
    var div = L.DomUtil.create('div', 'clustering');
    div.innerHTML = '<form><input id="clustering" ' + 'type="checkbox" checked/>clustering</form>';
    return div;
  };
...
// create list of maps from json returned
function createMapList(data, target) {
  var items = [];
  $.each(data.items, function(key, val) {
    var html = '<div class="map-box" data-map_id="' + val.id + '">' + '<img aria-hidden="true" src="' + W_LEAFLET.base_url +
      '/maps/thumb/' + val.id + '" />' + '     <div>' + '        <h4>' + decodeURIComponent(val.title.replace(/_/g,
        ' ')) + '</h4>' + '         <p>' + decodeURIComponent(val.description) + '</p>' + '     </div>' + '</div>';
...
function createLayerDiv(data) {
  ...
  // create
  $("#layer_list").prepend('<div class="img_list_div" id="' + data.items.id + '" data-map_id="' + data.items.id +
    '"><img src="' + W_LEAFLET.base_url + '/maps/thumb/' + data.items.id +
    '" /><span class="handle"> </span><div class="delete">X</div><div class="mapinfo">' + decodeURIComponent(data.items
      .title.replace(/_/g, ' ')) + '</div><div class="slider"></div></div>');
